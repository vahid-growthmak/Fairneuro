            ...
            {steps.map((step, i) => {
              ...
              return (
                <div key={step.title} className="text-center">
                  <span
                    className={cn(
                      'relative z-10 mx-auto flex h-[26px] w-[26px] items-center justify-center rounded-full font-heading text-[12px] font-semibold ring-4 ring-white',
                      solidStep[accent],
                    )}
                  >
                    {i + 1}
                  ...
                  <h3 className="mx-auto mt-3 max-w-[13rem] font-heading text-[13.5px] font-semibold leading-snug text-navy">
                    {step.title}
                  </h3>
                  {step.desc && (
                    <p className="mx-auto mt-2 max-w-[14rem] text-[11.5px] leading-relaxed text-navy/62">
                      {step.desc}
                    </p>
                  )}
                </div>
    ...
    <section className={bg}>
      <div className="shell py-16 lg:py-20">
        <div className="rounded-2xl bg-white/70 p-8 shadow-card lg:p-10">
          {title && (
            <h2 className="mb-9 text-center font-display text-[24px] font-semibold text-navy sm:text-[27px]">
              {title}
            </h2>
          )}
          <div className="grid items-start gap-y-9 sm:grid-cols-2 lg:grid-cols-[repeat(auto-fit,minmax(0,1fr))]">
            {steps.map((step, i) => {
              const accent = step.accent ?? cycle(stepCycle, i);
              return (
          ...
          {steps.map((step, i) => {
            ...
            return (
              ...
              >
                <span
                  className={cn(
                    'absolute left-4 top-4 flex h-[24px] w-[24px] items-center justify-center rounded-full font-heading text-[11.5px] font-semibold',
                    solidStep[accent],
                  )}
                >
                  {i + 1}
                ...
                {step.desc && (
                  <p className="mt-2.5 text-[11.5px] leading-relaxed text-navy/62">{step.desc}</p>
                )}
